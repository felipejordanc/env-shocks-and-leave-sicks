...
format week %tw
gen max_state = 1
replace max_state = 2 if state == "Regular"
replace max_state = 3 if state == "Alerta"
replace max_state = 4 if state == "Preemergencia"

gen pre_emerg = max_state == 4
gen alert = max_state == 4 | max_state == 3
gen regular = max_state == 4 | max_state == 3 | max_state == 2
gen mean_state = max_state
gen suma = 1
collapse (max) max_state (sum) alert pre_emerg regular suma (mean) mean_state, by(week)
...
label define state_lbl ///
    ...
    3 "Alerta" ///
    4 "Preemergencia"
label values max_state state_lbl
...
replace alert = alert/suma
replace pre_emerg = pre_emerg/suma
replace regular = regular/suma
...
drop if cod_comuna >13000 & cod_comuna < 14000
save "$usedata/pollution_input3.dta", replace
...
foreach m in MP10 MP25 NOX O3{
	preserve
	merge n:1 cod_comuna date using "$usedata/pollution_input3.dta", keep(1 3) nogenerate keepusing(mean`m' maxMP25 maxMP10 maxNOX maxO3  median`m')
	gen week = wofd(date)
	format week %tw
	drop id_all year cod_comuna
	compress
...
save "$usedata/pollution_merge3.dta", replace
