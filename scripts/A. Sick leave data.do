...
bysort benef_enciptado: egen total = total(monto_renta_imponible_pesos)
xtile income_3rd = total, nq(3)
keep benef_enciptado income_3rd
duplicates drop benef_enciptado, force
save "$usedata/3rds_graph.dta", replace
clear all
set obs 1
...
gen date= mofd(date2)
format date %tm
merge n:1 benef_enciptado date using "$usedata/Employer.dta", keep(1 3) nogenerate keepusing(monto_renta_imponible_pesos)
drop date
rename date2 date
collapse (mean) tmin tmax monto_renta_imponible_pesos, by(cod_comuna date)
bysort cod_comuna: egen meantmin = mean(tmin)
bysort cod_comuna: egen meantmax = mean(tmax)
bysort cod_comuna: egen meaning = mean(monto_renta_imponible_pesos)
xtile com_inc_3rd = meaning, nq(3)
xtile tmax_3rd = meantmax, nq(3)
xtile tmin_3rd = meantmin, nq(3)
keep cod_comuna com_inc_3rd tmax_3rd tmin_3rd
duplicates drop cod_comuna, force
save "$usedata/3rds_graph_com.dta", replace
