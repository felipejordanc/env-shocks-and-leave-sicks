...
counts_df <- tibble(
  bin = c(
    "dum_9", "dum_10_11", "dum_12_13", "dum_14_15",
...
counts_df <- tibble(
  ...
  Cold = c(
    3113258, 4158665, 6748854, 8144690, 7487207, 7068037,
    5837599, 4029771, 2198035, 1166360, 543231, 175916,
    75436, 51366
  ),
  Temperate = c(
    426917, 1785924, 4560751, 6440069, 8005358, 8892405,
    8544779, 8048482, 6370793, 4397696, 3713759, 2687096,
    1689062, 711578
  ),
  Hot = c(
    375073, 1408357, 3345797, 6982417, 9359069, 10542193,
    10678978, 11026270, 11119303, 12539000, 14749792,
    15312697, 9587655, 3852056
  ),
  total = c(
    3915248, 7352946, 14655402, 21567176, 24851634, 26502635,
    25061356, 23104523, 19688131, 18103056, 19006782,
    18175709, 11352153, 4615000
  )
)
...
counts_long <- counts_df |>
  pivot_longer(
    cols = c(Cold, Temperate, Hot, total),
    names_to = "group",
    values_to = "count"
  ) |>
  mutate(
    ...
    group = factor(group, levels = c("Cold", "Temperate", "Hot", "total"))
  )
...
tex_file <- "C:/Users/black/Documents/GitHub/env-shocks-and-leave-sicks/tables/income_tmax_com.tex"
raw <- readLines(tex_file)
...
df_tmax <- read_dta(
  "C:/Users/black/Dropbox/Sick Leave, Heat, Pollution/data/B_intermediate/3rds_graph_com.dta"
) |>
  mutate(cod_comuna = as.character(cod_comuna))

comunas_sf <- st_read(
  "C:/Users/black/Downloads/Comunas/comunas.shp",
  quiet = TRUE
) |>
  mutate(cod_comuna = as.character(cod_comuna))

comunas_merged <- comunas_sf |>
  left_join(df_tmax, by = "cod_comuna")



comunas_merged <- comunas_merged |>
  mutate(
    tmax_3rd = factor(
      tmax_3rd,
      levels = c(1, 2, 3),
...
p <- ggplot(comunas_merged) +
  geom_sf(aes(fill = tmax_3rd), color = "white", linewidth = 0.1) +
  scale_fill_manual(
    values = c(
      "Cold" = "#9ecae1",
      "Temperate" = "#a1d99b",
...
tex_file <- "C:/Users/black/Documents/GitHub/env-shocks-and-leave-sicks/tables/y_sick0_pol_tot_dum_ym.tex"
raw <- readLines(tex_file)
...
coef_rows <- raw[str_detect(raw, "^dum")]
se_rows   <- raw[str_detect(raw, "^\\s*&\\s*\\(")]
...
models <- c("maxMP10" ,"maxMP10_mean", "maxMP25", "maxMP25_mean", "meanMP10", "meanMP10_max")
